...
public class DijkstraTest {
    /**
     *   1. 다익스트라 알고리즘은 음의 간선이 없을때 노드 하나로부터 각각 다른 노드들까지의 거리를 모두 구하는 알고리즘
     *      - 다익스트라 알고리즘은 음의 간선이 있는 경우 모순이 생긴다. 로직을 생각해봤을때 방문, 즉 최단 거리를 확정
     *        지으려는 노드는 후보 노드들 중 가장 작은 값이다. 이렇게 노드의 최단 거리를 확정 지어놓고 음의 간선이 있는
     *        경우, 이미 확정된 노드까지의 거리보다 비용이 더 작은 경로가 있을 수 있다는 것이다.
     *   2. 초기 모델은 O(V^2) 이었지만, 방문하기 위한 가장 짧은 거리를 가진 노드를 찾는 로직을 우선순위큐(힙)를
